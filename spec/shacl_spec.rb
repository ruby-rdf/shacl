...
describe SHACL do
  describe ".get_shapes" do
    it "gets shapes from graph" do
      g = RDF::Graph.new
      expect(SHACL::Shapes).to receive(:from_graph).with(g)
      SHACL.get_shapes(g)
    end
  end
  ...
  describe ".open" do
    it "opens shapes graph and gets shapes" do
      g = RDF::Graph.new
      expect(RDF::Graph).to receive(:load).with("http://example.org/shapes_graph").and_yield(g)
  ...
  describe ".execute" do
    it "opens input and queries against queryable" do
      ...
      expect(SHACL).to receive(:open).with("http://example.org/shapes_graph").and_return(shapes)
      expect(shapes).to receive(:execute).with(g)
    ...
    it "opens input and queries against an empty graph" do
      shapes = double(:shapes)
      expect(SHACL).to receive(:open).with("http://example.org/shapes_graph").and_return(shapes)
      expect(shapes).to receive(:execute).with(RDF::Graph)
